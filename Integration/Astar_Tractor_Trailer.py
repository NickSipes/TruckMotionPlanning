...
def calculate_dist_policy(goalX, goalY, obstacleX, obstacleY, gridResolution, vehicleRadius):
	...
	obstacleMap, minX, minY, maxX, maxY, xWidth, yWidth = calc_obstacle_map(obstacleX, obstacleY, gridResolution, vehicleRadius)
	...
	openList.update({calc_index(goalNode, xWidth, minX, minY) : goalNode})
	...
	while True:
		...
		for i in range(numberOfMotions - 1):
			...
			currentNodeKey = calc_index(searchNode, xWidth, minX, minY)
			...
			if currentNodeKey in openList.keys():
				...
				# Add to open set
				openList.update({currentNodeKey : searchNode})
